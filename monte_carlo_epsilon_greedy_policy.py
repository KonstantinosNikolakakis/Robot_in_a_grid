### Monte-Carlo Control problem, deterministic starting state = START, stochastic actions to allow exploration
...
### Randomized policy to encourage exploration
def random_action(s,a,eps=0.1): 
	p=np.random.random()
	if p< 1- eps:
		return a 
	else:
		random_index = np.random.choice(np.arange(len(actions[states.index(s)]))) #Choose randomnly one of the allowed next positions
		return actions[states.index(s)][random_index]


def play_game(grid,policy,non_terminal_states): 

	s=START # set as staring position the START
	grid.set_state(s) 

	a=random_action(s,policy[s]) # stochastic action to allow exploration
	...
	while True: # play the game with random initial state according to the policy
		...
		if s in seen_states: # if the state is already seen penalize the corresponding action to encourage exclusion of loops in the path 
			...
			a=random_action(s,policy[s]) # Randomized action
