...
WALL  = {(2,4),(2,3),(3,4),(3,3),(5,2),(5,3),(5,4),(10,9)} # wall position
	...
	while True: # repeat until policy does not change: there is no policy that can imporve the cost function
		...
		while True: # Repeat until values "convergence": policy evaluation step
			...
			for s in states:
				...
				new_v_acc=0 # to sum with respect to all possible resulting states, (expectation in Bellman equation)
				if (len(actions[states.index(s)])!=0): # Check for terminal or unreachable positions, they have no further action
					for a in actions[states.index(s)]: # In this problem the actions "coincide" with the next state
						if (a==policy[s]): 
							p=0.5 # transition kernel
						else:
							p=0.5/(len(actions[states.index(s)])-1) # transition kernel
						
						grid.set_state(s)		# Update the state 
						r=grid.make_move(a,False,False) # Make move, False = do not print the grid
						new_v_acc += p*(r + GAMMA * V[grid.current_state()]) # Bellman equation
					V[s]=new_v_acc
		...
		for s in states:
			if (len(actions[states.index(s)])!=0): # Check for terminal or unreachable positions, they have no further action
				...
				for a1 in actions[states.index(s)]: # For all possible actions at state s
					v=0
					for a2 in actions[states.index(s)]: # To find the expectation in Bellman equation
						if (a2==a1):
							p=0.5 # transition kernel
						else:
							p=0.5/(len(actions[states.index(s)])-1) # transition kernel
						grid.set_state(s)	# Update the state
						r=grid.make_move(a2,False,False) # Make move, False = do not print the grid
						v +=p*(r+GAMMA*V[grid.current_state()]) # Find V, Bellman equation

					if (v > largest_value):  # Check for improvement
						largest_value = v # We find a larger value
						new_a = a1 			# New better policy candidate
